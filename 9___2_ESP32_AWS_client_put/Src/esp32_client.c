...
void aws_client_delete(char *dest_buffer, const char* endpoint, const char* query_params){
	...
	/* Buffer to hold full endpoint */
	char full_endpoint[TEMP_BUFF_LNG_SZ] = {0};

	/* Clear any previous data in the ESP UART buffer */
	buffer_clear(esp32_port);

	/* Construct the full endpoint with query parameters if provided */
	if(query_params != NULL && strlen(query_params) > 0){
		snprintf(full_endpoint, sizeof(full_endpoint), "%s?%s", endpoint, query_params);
	}
	else{
		strncpy(full_endpoint, endpoint, sizeof(full_endpoint));
	}
...
void aws_client_get_json(char *dest_buffer, const char* endpoint, const char* json_payload){
	...
	/* Prepare HTTP GET request with json body */
	snprintf(request_buffer, sizeof(request_buffer),
			 "GET %s HTTP/1.1\r\n"
			 "Host: %s\r\n"
			 "Content-Type: application/json\r\n"
